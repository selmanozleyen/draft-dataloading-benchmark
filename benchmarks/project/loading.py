...
def read_lazy(path):
    ...
    adata = ad.AnnData(X=ad.experimental.read_elem_lazy(g["X"]), obs=ad.io.read_elem(g["obs"]))
...
class BatchedZarrDataset(IterableDataset):
    ...
    def _get_chunks(self):
        ...
        slices = [slice(int(start), int(end)) for start, end in zip(chunk_boundaries[:-1], chunk_boundaries[1:])]
